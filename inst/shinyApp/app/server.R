...
server <- function(input, output, session)
{
    ...
    {
        if(length(app.variables$fcs.files)>0 && input$tabs==paste0("t_",tab.id)) 
        {
            files.list <- list()
            selected.file <- list()
    ...
    output$t_1_files_main <- renderUI(  #CREATION DE L'UI DE CHAQUE FICHIER 
    {
        file.ui <- NULL
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_2")
        {
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_3")
        {
    ...
    observe( #UPDATE POPULATIONS LIST
    {
        ...
        if(length(app.variables$fcs.files)>0)
        {
            if(length(input[["t_3_file_list"]])>0 && input[["t_3_file_list"]] != "")
            {
                ...
                if(length(app.variables$fcs.files[[f.name]]) > 0 && length(app.variables$populations.list[[f.name]])>0)
                {
                    fcs.temp <- app.variables$fcs.files[[f.name]]
                    fcs.mat <- app.variables$output.matrices[[f.name]]
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_6")
        {
    ...
    observe( #UPDATE FILES LIST
    {
        ...
        if(length(app.variables$fcs.files)>0)
        {
            for(i in 1:length(app.variables$fcs.files))
            {
                if(length(app.variables$fcs.files[[i]])>0 && app.variables$fcs.files[[i]][["type"]] == "CTRL")
                {
                    files.list[[length(files.list)+1]] <- names(app.variables$fcs.files)[[i]]
                }
            }
    ...
    observeEvent(input[["t_6_mut_add"]], #OBSERVE EVENT DU BOUTON ADD MUTANT
    {
        if(length(app.variables$fcs.files)>0)
        {
            update.log("CREATING MUTANT COHORT")
            for(i in 1:length(app.variables$fcs.files))
            {
                if(length(app.variables$fcs.files[[i]])>0 &&
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_7")
        {
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_4_hist")
        {
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_4_jp")
        {
            shinyjs::show("t_4_hist_right")
        }
        else
    ...
    { 
        ...
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_4_jp")
        {
            if(!is.na(input$t_4_jp_markers_list)>0 && input$t_4_jp_markers_list != "")
            {
                for(i in 1:length(app.variables$fcs.files))
                {
                    if(length(app.variables$fcs.files[[i]])>0)
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_5")
        {
            nmb.files <- 0
            for(i in 1:length(app.variables$fcs.files))
            {
    ...
    output$t_5_ctrl_list <- renderUI(
    {
        ...
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_5") 
        {
            ctrl.ui <- lapply(1:length(app.variables$fcs.files), function(i)
            {
                ...
                if(length(app.variables$fcs.files[[i]])>0)
                {
                    if(app.variables$fcs.files[[i]][["type"]] == "CTRL")
                    {
                        tmp.ui <- tagList(
                            fluidRow
                            (
    ...
    {
        if(length(app.variables$fcs.files)>0 && input$t_5_ctrl_all) 
        {
            lapply(1:length(app.variables$fcs.files), function(i)
            {
                if(length(app.variables$fcs.files[[i]])>0)
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_8")
        {
            nmb.files <- 0
            for(i in 1:length(app.variables$fcs.files))
            {
    ...
    observe( #CLEAR UI
    {
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_9")
        {
            nmb.files <- 0
            for(i in 1:length(app.variables$fcs.files))
            {
        ...
        if(length(app.variables$fcs.files)>0 && input$tabs=="t_9") 
        {
            ...
            files.plot <- lapply(1:length(app.variables$fcs.files), function(f.id)
            {
                ...
                if(length(app.variables$fcs.files[[f.id]])>0)
                {
                    ...
                    {
                        selected <- input[[paste0("t_9_cb_",f.id)]]
                    }
                    tmp.ui <- tagList(
                        column
                        (
